...
sconsUtilsDir = os.path.realpath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
...
class Configuration:
    ...
    def __init__(self, cfgFile, headers=(), libs=None, hasSwigFiles=True,
        ...
        if self.root == sconsUtilsDir:
            # cfg file is hosted by sconsUtils: there is no eups product
            self.eupsProduct = None
            self.version = None
        else:
            if eupsProduct is None:
