...
class Configuration:
    ...
    def __init__(self, cfgFile, headers=(), libs=None, hasSwigFiles=True,
        ...
        # Process eups products
        self.eupsProduct = eupsProduct
        if not isinstance(self, ExternalConfiguration):
            if not self.eupsProduct:
                self.eupsProduct = self.name
                state.log.warn("EUPS product not declared. Inferring from name %s" %
                               self.eupsProduct)
