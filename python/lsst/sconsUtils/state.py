...
def _initOptions():
    ...
    SCons.Script.AddOption(
        "--prefix", dest="prefix", action="store", default=False, help="Specify the install destination"
    )
    SCons.Script.AddOption(
        "--setenv",
        dest="setenv",
    ...
    SCons.Script.AddOption(
        "--tag", dest="tag", action="store", default=None, help="Declare product with this eups tag"
    )
    SCons.Script.AddOption(
        "--verbose",
        dest="verbose",
...
def _initVariables():
    ...
    opts.AddVariables(
        ...
        SCons.Script.EnumVariable(
            "opt", "Set the optimisation level", 3, allowed_values=("g", "0", "1", "2", "3")
        ),
        SCons.Script.EnumVariable(
            "profile", "Compile/link for profiler", 0, allowed_values=("0", "1", "pg", "gcov")
        ),
...
def _initEnvironment():
    ...
    env = SCons.Script.Environment(ENV=ourEnv, variables=opts, toolpath=[toolPath], tools=["default", "cuda"])
    env.cfgPath = cfgPath
    #
